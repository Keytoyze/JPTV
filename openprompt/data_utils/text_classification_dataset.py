...
class WikiTextPretrainProcessor(DataProcessor):
    def __init__(self):
        super().__init__()
        self.labels = ["", "", "", ""]
    
    def get_examples(self, data_dir, split):
        # return self.get_examples_pretrain(data_dir, split)
        return self.get_examples_pretrain_nsp(data_dir, split)

    def get_examples_pretrain_nsp(self, data_dir, split):
        ...
        for i in dataset['train']:
            ...
            if len(text) >= 100 and len(text) <= 1024 and not text.startswith("="):
                texts.append(text)
        ...
        for i, text in enumerate(texts):
            ...
            if len(splited) == 1:
                continue
            i = len(splited) // 2
            text_a, text_b = splited[:i], splited[i:]
            text_a = ". ".join(text_a) + "."
            text_b = ". ".join(text_b)
            if len(text_a) > 500 or len(text_b) > 500:
                continue

            example = InputExample(guid=str(idx * 2), text_a=text_a, text_b="", label=idx % 4)
            examples.append(example)
            example = InputExample(guid=str(idx * 2 + 1), text_a=text_b, text_b="", label=idx % 4)
            examples.append(example)
            idx += 1
        return examples
...
class AgnewsProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split):
        return self.get_examples_normal(data_dir, split)
        # return self.get_examples_pretrain(data_dir, split)
        # return self.get_examples_pretrain_nsp(data_dir, split)
    ...
    def get_examples_pretrain_nsp(self, data_dir, split):
        ...
        dataset = load_dataset("wikitext", 'wikitext-2-raw-v1', cache_dir="datasets/huggingface")
        examples = []
        print("dataset size:", len(dataset['train']))
        texts = []
        for i in dataset['train']:
    ...
    def get_examples_pretrain(self, data_dir, split):
        from datasets import load_dataset
        ...
        examples = []
        augs = [
            # naw.ContextualWordEmbsAug(model_path='roberta-large', model_type='roberta', device='cuda:0', aug_max=None),
            naw.SynonymAug(aug_max=None),
            naw.RandomWordAug(),
            # naw.BackTranslationAug(device='cuda:0')
        ]
        print("dataset size:", len(dataset['train']))
        texts = []
        # for i in tqdm(range(1000 if split == 'train' else 20)):
        for i in dataset['train']:
            text = i['text'].replace(" @", "").replace("@ ", "").strip()
            if len(text) >= 100 and len(text) <= 500 and not text.startswith("="):
                texts.append(text)
        random.shuffle(texts)
        breakpoint()
        if split != 'train':
        ...
        for i, text in enumerate(texts):
            ...
            aug1 = random.choice(augs).augment(text_a)[0]
            aug2 = random.choice(augs).augment(text_b)
            if len(aug2) != 0:
                aug2 = aug2[0]
            else:
                aug2 = text_b
            example = InputExample(guid=str(i * 2), text_a=text_a, text_b=text_b, label=i % 4)
            examples.append(example)
            example = InputExample(guid=str(i * 2 + 1), text_a=aug1, text_b=aug2, label=i % 4)
            examples.append(example)
        return examples

    def get_examples_normal(self, data_dir, split):
        # if split == 'train':
        #     split = 'train_wikitext_aug_syn_ran'
        path = os.path.join(data_dir, "{}.csv".format(split))
        examples = []
        with open(path, encoding='utf8') as f:
            reader = csv.reader(f, delimiter=',')
...
class TRECProcessor(DataProcessor):
    ...
    def __init__(self):
        super().__init__()
        self.labels = ["Abbreviation", "Entity", "Description and abstract concept", "Human being", "Location", "Numeric value"]

    def get_examples(self, data_dir, split):
        from datasets import load_dataset
        trec_dataset = load_dataset("trec", cache_dir=data_dir)
        examples = []
        if split == 'dev':
            raise FileNotFoundError()

        for i, x in enumerate(trec_dataset[split]):
            text_a = x['text']
            label = x['coarse_label']
            examples.append(InputExample(
                guid=str(i), text_a=text_a, label=label
            ))
        return examples


class GuardianAuthorshipProcessor(DataProcessor):
    """
    GuardianAuthorship Classification Dataset
    ...
    def get_examples(self, data_dir, split):
        from datasets import load_dataset
        if split == "train":
            split_text = "train[:60%]+validation[:60%]+test[:60%]"
        elif split == "test":
            split_text = "train[-40%:]+validation[-40%:]+test[-40%:]"
        else:
            raise FileNotFoundError()
        ga_dataset = load_dataset("guardian_authorship", name="cross_genre_1", cache_dir=data_dir, split=split_text)
        examples = []

        label_count = defaultdict(lambda : 0)

        for i, x in enumerate(ga_dataset):
            text_a = x['article']
            label = x['author']
            label_count[label] += 1
            examples.append(InputExample(
                guid=split + str(i), text_a=text_a, label=label
            ))
        
        logger.info(f"{split} label: {label_count}")
        return examples


class GuardianAuthorshipTopicsProcessor(DataProcessor):
    """
    GuardianAuthorship Classification Dataset
    ...
    def __init__(self):
        super().__init__()
        self.labels = ["Politics", "Society", "UK", "World", "Books"]
    
    def get_examples(self, data_dir, split):
        from datasets import load_dataset
        ga_dataset = load_dataset("guardian_authorship", name="cross_genre_1", cache_dir=data_dir, split="train+validation+test")
        examples = []

        label_count = defaultdict(lambda : 0)

        random.seed(0)
        indices = list(range(len(ga_dataset)))
        random.shuffle(indices)
        if split == "train":
            indices = indices[:int(len(indices) * 0.6)]
        elif split == "test":
            indices = indices[int(len(indices) * 0.6):]
        else:
            raise FileNotFoundError()
...
PROCESSORS = {
    ...
    "trec": TRECProcessor,
    "wikitext-pretrain": WikiTextPretrainProcessor,
    "guardianauthor": GuardianAuthorshipProcessor,
    "guardiantopics": GuardianAuthorshipTopicsProcessor
}
