...
class FewShotSampler(object):
    ...
    def __init__(self,
                 ...
                 noisy_samples: Optional[int]=None
                 ):
    ...
    def _sample(self,
        ...
        if seed is not None:
            ...
            self.rng2 = np.random.RandomState(seed * 7 - 1)
        else:
            self.rng = np.random.RandomState()
            self.rng2 = np.random.RandomState()
        ...
        if self.num_examples_per_label is not None:
            ...
            selected_ids, noisy_map = self.sample_per_label(indices, labels, self.num_examples_per_label) # TODO fix: use num_examples_per_label_dev for dev
        else:
        ...
        if sample_twice:
            ...
            if self.num_examples_per_label_dev is not None:
                ...
                selected_ids_dev, _ = self.sample_per_label(remain_ids, remain_labels, self.num_examples_per_label_dev)
            else:
            ...
            if isinstance(data, Dataset):
                ...
                for i in selected_ids:
                    if i in noisy_map:
                        target_idx, noisy_label = noisy_map[i]
                        x = data[target_idx]
                        assert data[i].label == noisy_label
                        x.label = noisy_label
                    else:
                        x = data[i]
                    result.append(x)

                return result, [data[i] for i in selected_ids_dev]
    ...
    def sample_per_label(self, indices: List, labels, num_examples_per_label):
        ...
        for label, ids in ids_per_label.items():
            ...
            selected = tmp[:num_examples_per_label].tolist()
            selected_ids.extend(selected)
            selected_label_to_ids[label] = selected
        ...
        if self.noisy_samples is not None and self.noisy_samples > 0:

            noisy_sample_map = {}
            target_ids = set()
            label_set = list(ids_per_label.keys())
            self.rng2.shuffle(label_set)
            noisy_labels = label_set[:self.noisy_samples]

            for noisy_label in noisy_labels:
                ids = selected_label_to_ids[noisy_label]
                self.rng2.shuffle(ids)
                noisy_id = ids[0]
                target_label = noisy_label
                while target_label == noisy_label:
                    self.rng2.shuffle(label_set)
                    target_label = label_set[0]
                ids = ids_per_label[target_label]
                target_id = ids[0]
                while target_id in selected_ids or target_id in target_ids:
                    self.rng2.shuffle(ids)
                    target_id = ids[0]
                target_ids.add(target_id)
                noisy_sample_map[noisy_id] = [target_id, noisy_label]
                logger.warning(f"Selected noisy id: {noisy_id} -> {target_id}, label: {noisy_label} -> {target_label}")
            return selected_ids, noisy_sample_map
        return selected_ids, {}
