...
class MixedTemplate(Template):
    ...
    def prepare(self):
        ...
        **Note that ``"soft_id"`` should have index start from 1 but not 0**
        ...
        for soft_id, token_id in emb_mp.items():
            print(f"{soft_id} -> {token_id}")
            self.soft_embedding.weight.data[soft_id, :] = self.raw_embedding.weight.data[token_id, :].clone().detach().requires_grad_(True)
        
        if self.ckpt_path is not None:
            self.load_state_dict(torch.load(self.ckpt_path, map_location='cpu'), strict=False)
            logger.info(f"load ckpt: {self.ckpt_path}")
    ...
    def incorporate_text_example(self,
        ...
        for i, d in enumerate(text):
            if 'placeholder' in d:
                text[i] = d["add_prefix_space"] + eval(d.get("post_processing", "lambda x:x"))(getattr(example, d['placeholder']))
            elif 'meta' in d:
                text[i] = d["add_prefix_space"] + eval(d.get("post_processing", "lambda x:x"))(example.meta[d['meta']])
            elif 'soft' in d:
    ...
    def soft_vac(self):
        content = []
        for i in range(len(self.soft_embedding.weight)):
            diff = self.soft_embedding.weight[i, :] - self.raw_embedding.weight
            score = torch.sum(diff ** 2, dim=-1)
            min_id = torch.argmin(score)
            min_score = score[min_id]
            min_word = self.tokenizer.decode([min_id.cpu().item()])
            content.append(f"Soft verb token {i} - {min_id}, {min_score}, {min_word}")
        return "\n".join(content)
