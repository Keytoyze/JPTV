...
class BaseRunner(object):
    ...
    def configure_optimizers(self):
        ...
        if not self.config.plm.optimize.freeze_para:
            ...
            for n, p in self.inner_model.plm.named_parameters():
                logger.info(f"plm_optimizer params: {n}, shape = {p.shape}, requires_grad = {p.requires_grad}")
        ...
        if hasattr(template_config, "optimize") and template_config.optimize is not None: # TODO should add optimize config in each yaml
            if not hasattr(self.inner_model.template, "optimize"):
                ...
                for n, p in self.inner_model.template.named_parameters():
                    logger.info(f"template_optimizer params: {n}, shape = {p.shape}, requires_grad = {p.requires_grad}")
        ...
        if hasattr(self.inner_model, "verbalizer") and self.inner_model.verbalizer:
            ...
            if hasattr(verbalizer_config, "optimize") and verbalizer_config.optimize is not None:
                if not hasattr(self.inner_model.verbalizer, "optimize"):
                    ...
                    for n, p in self.inner_model.verbalizer.named_parameters():
                        logger.info(f"verbalizer_optimizer params: {n}, shape = {p.shape}, requires_grad = {p.requires_grad}")
    ...
    def training_epoch(self, epoch):
        ...
        with tqdm(total=self.steps_per_epoch, desc=f"train epoch: {epoch}") as pbar:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if self.config.train.train_verblizer == "alternate2":
    ...
    def fit(self, ckpt: Optional[str] = None):
        ...
        if self.config.train.calibrate:
            cc_logits = calibrate(self.model, self.train_dataloader)
            print("the calibration logits is", cc_logits)
            self.model.verbalizer.register_calibrate_logits(cc_logits)
